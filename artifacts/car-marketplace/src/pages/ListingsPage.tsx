  ...
  return (
    <AppLayout>
      <ListingsHeader
        ...
        clearSearch={() => { setSearch(""); setMake(""); setModel(""); }}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6">
          ...
          {vehicleType && (
            <FilterChip label={getVehicleLabel(vehicleType)} onRemove={() => setVehicleType("")} />
          )}
          {condition && (
            <FilterChip label={getConditionLabel(condition)} onRemove={() => setCondition("")} />
          )}
          {(priceMin || priceMax) && (
            <FilterChip
          ...
          {make  && <FilterChip label={`${fx.makePlaceholder.split(" ")[0]}: ${make}`}  onRemove={() => setMake("")}  />}
          {model && <FilterChip label={`${fx.modelPlaceholder.split(" ")[0]}: ${model}`} onRemove={() => setModel("")} />}
          {(yearMin || yearMax) && (
            <FilterChip label={`${yearMin || "Any"} – ${yearMax || "Any"}`} onRemove={() => { setYearMin(""); setYearMax(""); }} />
          )}
