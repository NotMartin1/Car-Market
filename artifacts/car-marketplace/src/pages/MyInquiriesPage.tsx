...
import { getMyInquiries } from "@/lib/mock-api";
...
export default function MyInquiriesPage() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      getMyInquiries()
        .then((res) => setInquiries(res.inquiries as EnrichedInquiry[]))
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            {inquiries.map((inquiry) => (
              <div
                key={inquiry.id}
                className="bg-card rounded-3xl p-6 border border-border shadow-sm flex flex-col md:flex-row gap-6 hover:border-border transition-colors"
              >
          ...
          <div className="text-center py-20 bg-card rounded-3xl border border-dashed border-border">
            ...
            <Link href="/listings">
              <Button className="mt-6">Browse Cars</Button>
            </Link>
          </div>
        )}
      </div>
