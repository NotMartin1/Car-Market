...
import { MapPin, Fuel, Gauge, CheckCircle2 } from "lucide-react";
import { formatPrice, formatMileage } from "@/lib/utils";
import { Badge } from "@/components/ui/badge";

export function CarCard({ listing }: { listing: Listing }) {
  // Use seeded image or a high quality unsplash fallback
  const mainImage = listing.images?.[0] || "https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=800&q=80";

  return (
    <Link href={`/listings/${listing.id}`}>
      <div className="group bg-card rounded-2xl border border-border/50 overflow-hidden shadow-sm hover:shadow-xl hover:border-border transition-all duration-300 flex flex-col h-full cursor-pointer bg-white">
        
        {/* Image Container */}
        <div className="relative aspect-[4/3] overflow-hidden bg-muted">
          {/* subtle abstract fallback gradient if image fails, though img tag handles mostly */}
          ...
          <div className="absolute bottom-3 right-3 bg-background/90 backdrop-blur-md px-3 py-1.5 rounded-lg shadow-lg">
            <p className="font-display font-bold text-lg text-primary">{formatPrice(listing.price)}</p>
          </div>
        </div>
